local Library = (function()
    ...
    function Lib:Window(Configs)
        ...
        local KeySystem = Configs.Key and Configs.Key.KeySystem or false
        local KeyTitle = Configs.Key and Configs.Key.Title or "Key System"
...
local KeyNotifications = Configs.Key and Configs.Key.Notifi and Configs.Key.Notifi.Notifications or true
        ...
        function TabHandler:Tab(text)
            ...
            local Elements = {}
            setmetatable(Elements, {__index = TabHandler})
            
            function Elements:Button(Configs, callback)
                local text = ""
                local infoText = nil
                local actualCallback = nil

                if type(Configs) == "table" then
                    text = Configs.Title or Configs.text or "Button"
                    infoText = Configs.Info or Configs.Tooltip
                    actualCallback = Configs.Callback or callback
                else
                    text = tostring(Configs or "Button")
                ...
                Button.MouseButton1Click:Connect(function()
                    ...
                    if actualCallback then pcall(actualCallback) end
                end)

                if infoText then
                    self:Tooltip(Button, infoText)
                end
                return Button
            end

            function Elements:Toggle(Configs, default, callback)
                local text = ""
                local infoText = nil
                local toggled = false
                local actualCallback = nil
                ...
                if type(Configs) == "table" then
                    text = Configs.Title or Configs.text or "Toggle"
                    infoText = Configs.Info or Configs.Tooltip
                    toggled = Configs.Default or false
                    actualCallback = Configs.Callback or callback
                else
                    text = tostring(Configs or "Toggle")
                ...
                ToggleFrame.MouseButton1Click:Connect(function()
                    ...
                    if actualCallback then pcall(actualCallback, toggled) end
                end)

                if infoText then
                    self:Tooltip(ToggleFrame, infoText)
                end
                return ToggleFrame
            end

            function Elements:viewInfo(titleText, descText)
                ...
                InfoFrame.BackgroundColor3 = Color_Sec
                InfoFrame.Size = UDim2.new(1, 0, 0, 32)
                InfoFrame.Text = ""
                InfoFrame.AutoButtonColor = false
                ...
                InfoDesc.Position = UDim2.new(0, 35, 0, 32)
                InfoDesc.Size = UDim2.new(1, -45, 0, 0)
                InfoDesc.Font = Enum.Font.Gotham
                InfoDesc.Text = descText or ""
                InfoDesc.TextColor3 = Color_TextDim
                ...
                InfoFrame.MouseButton1Click:Connect(function()
                    ...
                    if expanded then
                        TweenService:Create(InfoFrame, TweenInfo.new(0.3, Enum.EasingStyle.Quart, Enum.EasingDirection.Out), {Size = UDim2.new(1, 0, 0, InfoDesc.AbsoluteSize.Y + 42)}):Play()
                        TweenService:Create(InfoStroke, TweenInfo.new(0.3), {Color = Color_Accent}):Play()
                    else
                        TweenService:Create(InfoFrame, TweenInfo.new(0.3, Enum.EasingStyle.Quart, Enum.EasingDirection.Out), {Size = UDim2.new(1, 0, 0, 32)}):Play()
                        TweenService:Create(InfoStroke, TweenInfo.new(0.3), {Color = Color3.fromRGB(45, 45, 45)}):Play()
                    end
                end)
            end
            ...
            function Elements:MultiDropdown(Configs)
                ...
                RefreshOptions()
                if Configs.Tooltip then self:Tooltip(Frame, Configs.Tooltip) end
            ...
            function Elements:Keybind(Configs)
                ...
                if Configs.Tooltip then
                    self:Tooltip(BindFrame, Configs.Tooltip)
                end
